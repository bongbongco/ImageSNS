 ...
 */
public class ImageListFragment extends android.support.v4.app.Fragment{
    ...
    public static ImageListFragment newInstance(int content_type, boolean add_padding) {
    ...
    public static ImageListFragment newInstance(int content_type, boolean add_padding, String param1) {
        Bundle args = new Bundle();
        args.putBoolean(EXTRA_ADD_PADDING, add_padding);
        args.putInt(EXTRA_CONTENT_TYPE, content_type);
    ...
    private ArrayList<ContentBean> mItemList;
...
//    private int before_first_item = 0;
    ...
    private void loadByContentType(int type, Context context, int page, ApiCaller.ApiListener listener) {
        switch (type) {
            case CONTENT_DEFAULT:
                ApiCaller.getInstance().getImageList(context, page, listener); break;
            case CONTENT_TAG:
                ApiCaller.getInstance().getImageListByTag(context, page, CONTENT_PARAM1, listener); break;
            case CONTENT_USER_UPLOAD:
                //ApiCaller.getInstance().getUserUploadList(context, page, CONTENT_PARAM1, listener); break;
                ApiCaller.getInstance().getImageList(context, page, listener); break;
            case CONTENT_WELCOME:
                ApiCaller.getInstance().getWelcomeList(context, page, listener); break;
            default:
                ApiCaller.getInstance().getImageList(context, page, listener); break;
        }
    }
    ...
    private void initContentInfo(int content_type) {
        ...
        CONTENT_PARAM1 = getArguments().getString(EXTRA_PARAM1);
        switch (CONTENT_TYPE) {
            case CONTENT_DEFAULT:
                LIST_COL_NUM = 2;
                break;
    ...
    /**
     * Invalidate list item like when user removed item from list on runtime.
     * ArrayAdapter is watching validity of item and if not valid then do not
     * display its content image.
     * @param position position of item
     * @param valid new validity to set
     */
    protected void setItemValidity(int position, boolean valid) {
        Log.i("setItemValidity", "changed validity of item(position = "+position+") to "+valid);
        mItemList.get(position).valid = valid;
    }

    protected boolean getItemValidity(int position) {
        return mItemList.get(position).valid;
    }
    ...
    private void initList(View root) {
        ...
        mGridView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ContentBean bean = mAdapter.getItem(position);
                Gson gson = new Gson();
        ...
        mItemList = new ArrayList<ContentBean>();
    ...
    private void retrieveJson(String response) throws JSONException, DuplicateFormatFlagsException{
        ...
        JSONArray itemArray;
        //try {
            itemArray = root_jo.getJSONArray("items");
//        } catch(JSONException e) {
//            e.printStackTrace();
//            return;
//        }
        ...
        if(load_item_num<20) end_flg = true;
        for(int i = 0 ; i < load_item_num ; i ++) {
            JSONObject item_jo = itemArray.getJSONObject(i);
            ContentBean item   = new ContentBean();
            item.id            = item_jo.getString("id");
            item.url           = item_jo.getString("url");
            // todo unify format
            try {item.width         = item_jo.getInt("item_flg");} catch (JSONException e){e.printStackTrace();}
            try {
                JSONObject user_jo = item_jo.getJSONObject("user");
                item.user_id = user_jo.getString("id");
                item.user_name = user_jo.getString("name");
                item.user_img_url = user_jo.getString("url");
            } catch (JSONException e) { Log.i("JSON ERROR", "no value for user");}
    ...
    private AbsListView.OnScrollListener getScrollListener() {
        return new AbsListView.OnScrollListener() {
            ...
            public void onScroll(AbsListView view, int firstVisibleItem,
                ...
                //load next item
                if (!end_flg && !busy.get() && totalItemCount - firstVisibleItem <= 20) {
                    int page = totalItemCount / 20;
    ...
    private class ImageAdapter extends ArrayAdapter<ContentBean> {
        ...
        public ImageAdapter(Context context, List<ContentBean> objects) {
        ...
        public View getView(int position, View convertView,
            ...
            // set content
            ContentBean item = getItem(position);

            // if list item is removed in runtime, do not display its image
            if(item.valid) {
                ...
                holder.imageView.setTag(mImageLoader.get(item.url, listener));
            } else {
